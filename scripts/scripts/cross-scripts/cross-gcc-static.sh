...
# cross-lfs cross-gcc w static libgcc build
# -----------------------------------------
# $LastChangedBy$
# $LastChangedDate$
# $LastChangedRevision$
...
test -d ${SRC}/gcc-${GCC_VER}-cross-static &&
   rm -rf ${SRC}/gcc-${GCC_VER}-cross-static

mkdir -p ${SRC}/gcc-${GCC_VER}-cross-static &&
cd ${SRC}/gcc-${GCC_VER}-cross-static
...
#     LIBGCC2_CFLAGS="${TGT_CFLAGS}" all-gcc \
make CFLAGS_FOR_TARGET="-O2 -pipe ${TGT_CFLAGS}"  all-gcc \
   >> ${LOGFILE} 2>&1 &&
