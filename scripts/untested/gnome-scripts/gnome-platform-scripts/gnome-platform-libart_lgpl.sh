...
cd ${SRC}
LOG=libart_lgpl-gnome-platform.log
...
unpack_tarball libart_lgpl-${LIBART_LGPL_VER}
cd ${PKGDIR}

max_log_init libart_lgpl ${LIBART_LGPL_VER} "gnome-platform (shared)" ${CONFLOGS} ${LOG}
...
if [ "Y" = "${MULTIARCH}" ]; then
   use_wrapper /usr/bin/libart2-config
fi
