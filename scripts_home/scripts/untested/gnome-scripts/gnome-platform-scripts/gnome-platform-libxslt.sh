...
cd ${SRC}
LOG=libxslt-gnome-platform.log
...
unpack_tarball libxslt-${LIBXSLT_VER}
cd ${PKGDIR}

max_log_init libxslt ${LIBXSLT_VER} "blfs (shared)" ${CONFLOGS} ${LOG}
...
if [ "Y" = "${MULTIARCH}" ]; then
   use_wrapper /usr/bin/xslt-config
fi
